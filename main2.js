    ...
    drawUpgrades(player) {
        ...
        for(var key in upgrades) {
            if (player.upgrades[key]) {
                this.add.image(player == this.p1? MARGIN:CENTER_X + MARGIN, MARGIN+((LINEHEIGHT + 10)*drawn),key)
                if (!player.upgrades[key].displayText) {
                    player.upgrades[key].displayText = this.add.bitmapText(player == this.p1? MARGIN * 1.5 
                        : this.p2.LEFT + (MARGIN/2),LINEHEIGHT+((LINEHEIGHT+10)*drawn), "type-y","X" + player.upgrades[key].owned, 32)
                    player.upgrades[key].rateText = this.add.bitmapText(player == this.p1? this.p1.LEFT + 150 
                        : this.p2.LEFT + 150,LINEHEIGHT+((LINEHEIGHT+15)*drawn), "type-y", "+" + (upgrades[key].rate * player.upgrades[key].owned).toFixed(1) + " f/sec", 24)
                } else {
                    player.upgrades[key].displayText.y = LINEHEIGHT + ((LINEHEIGHT+10)*drawn)
                    player.upgrades[key].displayText.setText("X" + player.upgrades[key].owned);
                    player.upgrades[key].rateText.y = LINEHEIGHT + ((LINEHEIGHT + 15)*drawn);   
